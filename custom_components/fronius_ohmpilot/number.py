...
class OhmpilotMaxPowerNumber(OhmpilotBaseNumber):
    ...
    _attr_name = "Fronius Ohmpilot Output Power"
    ...
    def __init__(self, api_client: FroniusOhmpilotApiClient, entry: ConfigEntry) -> None:
        """Initialize with configurable max power."""
        super().__init__(api_client, entry)
        self._attr_native_max_value = float(
            entry.data.get(CONFIG_KEY_HEATER1_MAX_POWER, DEFAULT_HEATER1_MAX_POWER)
        )
...
class OhmpilotMaxTempNumber(OhmpilotBaseNumber):
    ...
    _attr_name = "Fronius Ohmpilot Target Temperature"
