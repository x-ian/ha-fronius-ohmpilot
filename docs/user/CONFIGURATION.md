...
This document describes all configuration options and settings available in the Fronius Ohmpilot custom integration.
...
### `fronius_ohmpilot.example_service`
...
```yaml
service: fronius_ohmpilot.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: fronius_ohmpilot.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
- [GitHub Issues](https://github.com//x-ian//fronius_ohmpilot/issues) - Report problems
