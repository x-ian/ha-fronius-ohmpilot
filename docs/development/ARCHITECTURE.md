...
This document describes the technical architecture of the Fronius Ohmpilot custom component for Home Assistant.

## Directory Structure

```text
custom_components/fronius_ohmpilot/
...
│   ├── __init__.py          # Exports FroniusOhmpilotDataUpdateCoordinator
...
│   ├── __init__.py          # Exports FroniusOhmpilotEntity
...
- `base.py` - Main coordinator class (`FroniusOhmpilotDataUpdateCoordinator`)
...
**Key class:** `FroniusOhmpilotDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `FroniusOhmpilotApiClient`
...
- `FroniusOhmpilotConfigFlowHandler` (main flow)
- `FroniusOhmpilotOptionsFlow` (options)
...
**Key class:** `FroniusOhmpilotEntity` (in `entity/base.py`)
...
2. `FroniusOhmpilotEntity` for common functionality
...
1. Create directory: `custom_components/fronius_ohmpilot/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `FroniusOhmpilotEntity`
